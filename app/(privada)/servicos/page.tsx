...
import { getEspecialidades } from "@/app/actions/especialidades/getEspecialidades";
...
const Servicos = () => {
  ...
  // Função que recarrega as especialidades quando refresh mudar
  useEffect(() => {
    const buscarEspecialidades = async () => {
      const result = await getEspecialidades();
      if (result.success) setEspecialidades(result.data);
    };
  ...
  const [serviceTypes] = useState([
    {
      ...
      description:
        "Limpeza de filtros, verificação de dreno e higienização da evaporadora.",
    },
    {
      id: 2,
      ...
      specialty: "Elétrica",
      description:
        "Montagem completa de QDC com barramentos e identificação de circuitos.",
    },
    {
      id: 3,
      ...
      description:
        "Localização e conserto de vazamentos em tubulações de PVC ou cobre.",
    },
    {
      id: 4,
      ...
      description:
        "Configuração de rede Wi-Fi, segurança WPA3 e controle parental.",
    },
    {
      id: 5,
      ...
      specialty: "Elétrica",
      description:
        "Substituição de disjuntor danificado por modelo compatível (DIN ou NEMA).",
    },
  ]);
    ...
    <div className="space-y-6 animate-in fade-in duration-500">
      ...
      <Tabs defaultValue="tipos" className="w-full">
        <TabsList className="bg-slate-100 p-1 rounded-xl mb-6 w-full sm:w-auto flex">
          <TabsTrigger
            value="tipos"
            className="flex-1 sm:flex-none cursor-pointer rounded-lg font-bold text-xs data-[state=active]:bg-white data-[state=active]:text-blue-600 data-[state=active]:shadow-sm"
          >
              ...
              <Table>
                <TableHeader className="bg-slate-50/50">
                  <TableRow className="hover:bg-transparent border-b border-slate-200">
                    <TableHead className="h-10 text-[9px] md:text-[11px] font-black uppercase tracking-widest text-slate-500 pl-4 md:pl-6">
                      Serviço / Descrição
                    </TableHead>
                    <TableHead className="h-10 text-[9px] md:text-[11px] font-black uppercase tracking-widest text-slate-500">
                      Especialidade
                    </TableHead>
                    <TableHead className="h-10 text-[9px] md:text-[11px] font-black uppercase tracking-widest text-slate-500 text-right pr-4 md:pr-6">
                      Ações
                    </TableHead>
                  </TableRow>
                </TableHeader>
